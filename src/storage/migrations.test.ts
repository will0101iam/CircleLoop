...
describe('migrations', () => {
  ...
  it('adds llm selection columns to legacy chat_threads tables', async () => {
    const run = vi.fn().mockResolvedValue(undefined)
    const query = vi
      .fn()
      .mockResolvedValueOnce([
    ...
    expect(run).toHaveBeenCalledWith('alter table chat_threads add column llm_provider text', [])
    expect(run).toHaveBeenCalledWith('alter table chat_threads add column llm_model text', [])
  })
